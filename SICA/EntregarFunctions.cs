            ...
            using (SQLiteConnection sqliteConnection = new SQLiteConnection("Data Source=" + Globals.DBPath))
            {
                ...
                try
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        strSQL = "UPDATE INVENTARIO_GENERAL SET CUSTODIADO = 'PRESTADO', USUARIO_POSEE = '" + Globals.UsernameSelect + "', FECHA_POSEE = " + fecha + " WHERE ID_REPORTE_VALORADOS_FK = " + row["ID_REPORTE"].ToString();
                        sqliteCmd = new SQLiteCommand(strSQL, sqliteConnection);
                        sqliteCmd.ExecuteNonQuery();
        ...
        public static bool EntregarPagaresCarrito(int desembolsado)
        {
            using (SQLiteConnection sqliteConnection = new SQLiteConnection("Data Source=" + Globals.DBPath))
            {
                ...
                try
                {
                    ...
                    if (desembolsado == 1)
                    {
                        strSQL = "SELECT TC.ID_REPORTE_VALORADOS_FK AS ID, '0' AS '#', RV.CIP, RV.NOMBRE, RV.MONTO, RV.SISGO_SOLICITUD AS 'SISGO'";
                    ...
                    {
                        strSQL = "SELECT TC.ID_PAGARE_SIN_DESEMBOLSAR AS ID, '0' AS '#', DESCRIPCION_2 AS 'DESCRIPCION 2', DESCRIPCION_3 AS 'DESCRIPCION 3', DESCRIPCION_4 AS 'DESCRIPCION 4', RV.SISGO_SOLICITUD AS 'SISGO'";
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        if (desembolsado == 1)
                        {
                            strSQL = "INSERT INTO PAGARE_HISTORICO (ID_USUARIO_ENTREGA_FK, ID_USUARIO_RECIBE_FK, SOLICITUD_SISGO, FECHA_INICIO, FECHA_FIN) VALUES (" + Globals.IdUsername + ", " + Globals.IdUsernameSelect + ", " + row["SISGO"].ToString() + ", " + fecha + ", " + fecha + ")";
                        }
                        else
                        {
                            strSQL = "INSERT INTO PAGARE_HISTORICO (ID_USUARIO_ENTREGA_FK, ID_USUARIO_RECIBE_FK, ID_REPORTE_VALORADOS_FK, SOLICITUD_SISGO, FECHA_INICIO, FECHA_FIN) VALUES (" + Globals.IdUsername + ", " + Globals.IdUsernameSelect + ", " + row["ID"].ToString() + ", " + row["SISGO"].ToString() + ", " + fecha + ", " + fecha + ")";
                        }
                        sqliteCmd = new SQLiteCommand(strSQL, sqliteConnection);
                        sqliteCmd.ExecuteNonQuery();
                    }
                    ...
                    GlobalFunctions.ArmarCargoExcel(dt, Globals.PlantillaCargoPath, Globals.CargoPath + "CARGO_PAG_" + DateTime.Now.ToString("yyyymmddhhmmss") + "_" + Globals.Username + ".xlsx", 4, 1, true);
