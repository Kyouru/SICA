...
namespace SICA.Forms.Busqueda.Historico
{
    public partial class HistoricoEdicion : Form
    {
        ...
        {
            string strSQL = "";
            strSQL = "SELECT FECHA_MODIFICA, U.NOMBRE_USUARIO AS USUARIO_MODIFICA, DEP.NOMBRE_DEPARTAMENTO AS DEPART, DOC.NOMBRE_DOCUMENTO AS DOC, FORMAT(FECHA_DESDE, 'dd/MM/yyyy') AS DESDE, FORMAT(FECHA_HASTA, 'dd/MM/yyyy') AS HASTA, DESCRIPCION_1 AS DESC_1, DESCRIPCION_2 AS DESC_2, DESCRIPCION_3 AS DESC_3, DESCRIPCION_4 AS DESC_4, DESCRIPCION_5 AS DESC_5, NUMERO_DE_CAJA";
            strSQL += " FROM ((INVENTARIO_ANTERIOR IA LEFT JOIN USUARIO U ON IA.ID_USUARIO_MODIFICA = U.ID_USUARIO)";
            ...
            strSQL += " UNION ALL";
            strSQL += " SELECT FECHA_MODIFICA, U.NOMBRE_USUARIO AS USUARIO_MODIFICA, DEP.NOMBRE_DEPARTAMENTO AS DEPART, DOC.NOMBRE_DOCUMENTO AS DOC, FORMAT(FECHA_DESDE, 'dd/MM/yyyy') AS DESDE, FORMAT(FECHA_HASTA, 'dd/MM/yyyy') AS HASTA, DESCRIPCION_1 AS DESC_1, DESCRIPCION_2 AS DESC_2, DESCRIPCION_3 AS DESC_3, DESCRIPCION_4 AS DESC_4, DESCRIPCION_5 AS DESC_5, NUMERO_DE_CAJA";
            strSQL += " FROM ((INVENTARIO_GENERAL IG LEFT JOIN USUARIO U ON IG.ID_USUARIO_MODIFICA = U.ID_USUARIO)";
            ...
            strSQL += " ORDER BY FECHA_MODIFICA ASC";
